...
const random = require('lodash/random');  // lodash per generare numeri casuali
...
class AmazonScraper {
    constructor(filterConfigPath, dataConfigPath, proxies = []) {
    ...
    async postDiscordWebhook(data) {
        try {
            ...
            const webhookUrls = await this.matchFilter(data);
                ...
                embeds: [
                    {
                        ...
                        color: this.filterConfig.embed_data.color,
                        fields: [
                            { name: 'Old Price', value: avgPrice, inline: true },
                            { name: 'New Price', value: currentPrice, inline: true },
                            { name: 'Discount', value: percentOff, inline: true },
                            { name: 'Link', value: `https://www.amazon.com/dp/${data.asin}`, inline: true },
                        ],
                        author: {
                            name: this.filterConfig.embed_data.author_name,
                            icon_url: this.filterConfig.embed_data.author_icon_url,
                        },
                        footer: {
                            text: this.filterConfig.embed_data.footer,
                            icon_url: this.filterConfig.embed_data.footer_icon,
                        },
                        image: { url: data.image },
                    },
                ],
                username: this.filterConfig.embed_data.username,
                avatar_url: this.filterConfig.embed_data.avatar_url,
            };

            for (const webhook of webhookUrls) {
                for (const url of webhook.webhook) {
                    await new Promise(resolve => setTimeout(resolve, random(2500, 3500)));

                    const response = await this.makeRequest('post', url, { 'Content-Type': 'application/json' }, payload);

                    if (response.status === 429 || response.status === 417) {
                        const index = this.alreadySeen.indexOf(data._id);
                        if (index > -1) this.alreadySeen.splice(index, 1);
                        await new Promise(resolve => setTimeout(resolve, 2000));
                    }

                    if (response.status !== 204) {
                        logger.error(`Failed to send webhook: ${response.status} - ${response.data}`);
                    }
                }
            }
        } catch (err) {
            logger.error(`Failed to create payload: ${err}`);
        }
    }
    ...
    async handleTask(category, price, age, priority) {
        ...
        for (const deal of parsedDeals) {
            ...
            await this.postDiscordWebhook(deal);
        }
    }
...
(async () => {
    ...
    const scraper = new AmazonScraper(configJson, dataJson, proxies);
    await scraper.main();
})();
